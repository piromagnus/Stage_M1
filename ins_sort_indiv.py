...
def diag(att_weight):
    ...
    ls=att_weight.shape[1]
    res=np.zeros((batch,ls))
    for i in range(ls):
...
def train_data(dataset,length,binary_size):
    for (batch,(inp,mask,tar)) in enumerate(dataset):
        ...
        dec_inp = tf.zeros((batch_size,state_size), dtype=tf.int64)
        ...
        for i in range(n_loop-1):
            pred,pred_pos=train_data_loop(i,inp,dec_inp,enc_padding_mask,combined_mask,dec_padding_mask,dec_inp,length,binary_size)
        #pointer=pred_pos[list(pred_pos)[-1]]
            train_mask_loop(i,enc_padding_mask,pred_pos)
        if batch % 500 == 0:
...
def train_data_loop(i,inp,dec,enc_mask,com_mask,dec_mask,dec_inp,length,binary_size):
    ...
    with tf.GradientTape() as tape:
        ...
        #print(enc_inp.shape,mask.shape)
        
        predictions,_,predicted_pos = transformer(enc_inp, dec, True, mask, com_mask, dec_mask)
        print(enc_inp[0],mask[0])
        #print(predictions.shape,predicted_pos.shape)
        ...
        loss_position = loss_pos(diag(predicted_pos),tf.squeeze(tf.cast(enc_mask[:,i+1,:,:],tf.float32)))#, weights)
    ...
    pred_binary = back2int(pred_binary)
    print(pred_binary[0],predicted_pos[0])
...
def train_mask_loop(i,mask,att_weight):
    ...
    pointer=tf.reshape(pointer,(pointer.shape[0],pointer.shape[1],1))
    #print(mask,pointer)
    mask_concat=tf.concat([mask[:,i,:,:],pointer],axis=-1)
    ...
    with tf.GradientTape() as tape:
        ...
        predict_msk = tf.reshape(predict_msk, (mask.shape[0], mask.shape[-2]))
    ...
    print(predict_msk_binary[0])
    err = tf.reduce_sum(tf.squeeze(mask[:,i+1,:,:])-predict_msk_binary, axis=-1)
