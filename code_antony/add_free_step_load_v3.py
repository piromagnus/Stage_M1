...
# Ce script permet à partir d'un réseau déjà entrainé dont on charge les poids, d'entraîné uniquement sur l'étape de transfert
# Avec nb_free_line ligne gratuites.

# Paramètre : numéro du GPU, VRAM max, nb epoch, taille des opérandes, restricted ou non, nombre de ligne gratuite, nom du fichier à charger (optionnel), graine aléatoire (optionnel), chaine à ajouter au nom du fichier de log (optionnel)
# Attention pour obtenir des résultats intéressant, il faut que la taille des opérandes et le restricted correspondent au réseau chargé.
...
if len(sys.argv) >  7:
    trained_model_name = sys.argv[7]
else:
    trained_model_name = None

nb_step_early_stopping = 50
accuracy_early_stopping = 0.5

step_between_save = 50

save_weight_at_best_epoch = True

if len(sys.argv) >  8:
    last_seed = sys.argv[8]
else:
    last_seed = None
...
# Par défaut il y a autant de lignes gratuites pour le transfert que ce qu'il y a d'étapes de multplication, soit autant que le nombre de chiffre des opérandes.
# Chaque ligne gratuite correspond à result_size vecteur composé que de 0 et 1 composé que de 1 (fin de ligne).
# Il n'est pas possible de mettre 0, car il n'y aurait plus de moyen pour le réseau de différencier le transfert de la première étape.
# Plus on augmente cette valeur au dessus de operand size, on augmente aussi la longueur des séquences et donc la RAM utilisé.
nb_free_line = int(sys.argv[6])
...
name = "free_step_{}digits_{}line_{}".format(result_size, nb_free_line, random_seed)
if len(sys.argv) > 9:
    name += "_" + sys.argv[9]
...
with open(log_name, "w+") as l:
    l.write(
        "Log pour l'apprentissage de multiplication {} par {} chiffres avec résultat de {} chiffre en transfert en utilsant {} lignes gratuites pour le transfert à partir d'un modèle entrainé.\n\n\n\n".format(
            operand_size, operand_size, result_size, nb_free_line))
...
def generate_data(list_x, list_y, nb_step, seq_in, seq_out, max_len, size, addition=False, batch=True, in_size=20,
                  add_total=0, space=False, free_line = None):
    """
    ...
    if addition:
        ...
        if batch:
            if add_total and free_line > nb_step - 1:
                seq_in = (free_line + 1) * seq_out
            ...
            if add_total == 1:
                if free_line is not None:
                    encoder_input[-1, :, : (free_line + 1) * seq_out, :] = np.zeros((1, len(list_x), (free_line + 1) * seq_out, in_size), dtype=np.uint8)
                else:
    ...
    for x, y, index in zip(list_x, list_y, range(len(list_x))):
        if addition:
            ...
            nb_line_transfer = nb_step
            if free_line is not None :
                nb_eol = free_line + 1
                if free_line > nb_step - 1:
                    nb_line_transfer = free_line + 1
            else:
                nb_eol = nb_step
            for z in range(nb_eol):
                encoder_input[-1, index, (z + 1) * (seq_in // nb_line_transfer) - 1] = np.ones(20)
...
encode_in, decode_in_list, encoder_input, decoder_input, decoder_output = generate_data(x_list, y_list, nb_step, seq_size, result_size + 1, operand_size, result_size, add_total=with_transfer, free_line=nb_free_line)

x_list_test = [x[0] for x in liste_B]
y_list_test = [x[1] for x in liste_B]
encode_in_test, decode_in_list_test, encoder_input_test, decoder_input_test, decoder_output_test = generate_data(x_list_test, y_list_test, nb_step, seq_size, result_size + 1, operand_size, result_size, add_total=with_transfer, free_line=nb_free_line)

x_list_validation = [x[0] for x in liste_C]
y_list_validation = [x[1] for x in liste_C]
encode_in_validation, decode_in_list_validation, encoder_input_validation, decoder_input_validation, decoder_output_validation = generate_data(x_list_validation, y_list_validation, nb_step, seq_size, result_size + 1, operand_size, result_size, add_total=with_transfer, free_line=nb_free_line)
...
# Pour avoir la durée des epochs
...
for i in range(start_epoch, nb_epochs):
    ...
    print("execution in {}s".format(int(time_callback.times)))
    ...
    detail_list_train = []
    detail_list_test = []
    rep, acc_train = get_acc(encoder_input[nb_step, val_train], din_list[val_train, nb_step - 1], result_size + 1)
    acc_list_train.append(acc_train)
    detail_list_train.append(rep)
    rep, acc_test = get_acc(encoder_input_validation[nb_step], din_list_validation[range(din_list_validation.shape[0]), nb_step - 1], result_size + 1)
    acc_list_test.append(acc_test)
    detail_list_test.append(rep)
    x_list = [x[0] for x in liste_C]
    y_list = [x[1] for x in liste_C]
    a = get_transfer_result(x_list, y_list, (nb_free_line + 1) * (result_size + 1), result_size)
    ...
    with open(log_name, "a") as l:
        l.write(
            "\n\nEpoch : {} exécuté en {} secondes\n {} Mo de RAM utilisé\nAccuracy on train set : {}\nAccuracy on validation set : {}\nDetail Accuracy on train set : {}\nDetail Accuracy on validation set : {}\nActiv learning distribution : {}\nAccuracy on validation set for tranfer operation : {}\n\n".format(
                str(i), str(int(time_callback.times)), str(process.memory_info().rss / 1000000), str(acc_list_train),
                str(acc_list_test),
...
for x in range(len(liste_B)// 1000):
    ...
    a = [int(x) for x in a]
    acc_transfer += sum([x == y for x, y in zip(a,b)])
acc_transfer = acc_transfer / (len(liste_B) // 1000 * 1000)

print("\nAccuracy pour l'opération transféré avec {} lignes gratuites (de traitement) : {}\n".format(str(nb_free_line), str(acc_transfer)))

with open(log_name, "a") as l:
    l.write("\nAccuracy pour l'opération transféré avec {} lignes gratuites (de traitement) : {}\n".format(str(nb_free_line), str(acc_transfer)))

os.rename(log_name, "log_{}_{}.txt".format(name, str(acc_transfer)))
os.rename(weight_name, "weight_{}_{}.h5".format(name, str(acc_transfer)))
