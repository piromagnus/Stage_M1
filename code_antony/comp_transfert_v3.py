...
operand_size = 3 #int(sys.argv[4]) # La taille maximal des opérandes
restricted = 0 #int(sys.argv[5])   # Si le resultat peut faire une taille 2 * operand_size ou seulement 2 * operand_size - 1
...
if not restricted:
    ...
    max_operand = int(10 ** operand_size * 0.31622776601683793319988935)
seq_size = nb_step * (result_size + 1)


name = "comp_transfer_{}digits_{}".format(result_size, random_seed)
...
for i in range(start_epoch, nb_epochs):
    ...
    val_train = [random.randint(0,train_set_size - 1) for i in range(100)]
    acc_list_test = []
    acc_list_train = []
    detail_list_train = []
    detail_list_test = []
    ...
    for ik in range(nb_step + with_transfer):
        ...
        if ik < nb_step:
            rep, acc_validation = get_acc(encoder_input_validation[ik],
                                          din_list_validation[range(din_list_validation.shape[0]), ik], result_size + 1)
        else:
            rep, acc_validation = get_acc(encoder_input_validation[ik],
                                          din_list_validation[range(din_list_validation.shape[0]), nb_step - 1],
                                          result_size + 1)
        acc_list_test.append(acc_validation)
        detail_list_test.append(rep)
    x_list = [x[0] for x in liste_C]
    y_list = [x[1] for x in liste_C]
    a = get_transfer_result(x_list, y_list, seq_size, result_size)
...
print("\nAccuracy pour l'opération transféré : {}\n".format(str(acc_transfer)))
